...
import BasicCard from './WeatherData';
...
function App(){
  ...
  const [getForecastDataHours, setForecastDataHours] = useState([]);
  ...
  const performApi = async (text) => {
    ...
    try {
      if(text.length > 0){
        const fetchApi = await axios.get(`https://api.weatherapi.com/v1/forecast.json?key=8ed60fb5871c4d04b17182001242207&q=${text}&days=1&aqi=yes&alerts=yes`);
        ...
        setForecastDataHours([...forecast.forecastday[0].hour]);
  ...
  return (
    <>
      ...
      <Box className="appSearch">
        
       <Input
          id="standard-basic"
          color='info' sx={{width: "20rem"}} placeholder='Bangalore, India'
          style= {{color: "#fff"}}
          startAdornment={
            <InputAdornment position="start">
              <TravelExploreRoundedIcon sx={{color: "white", fontSize: "2.3rem"}}/>
            </InputAdornment>
          }
